...
**Part (b)**
30240 cominitation
1 -probablity none out of 240 are the same
```{r}
#probablity none out of 240 are the same
p<-1-(30239/30240)^240
p
...
```{r}
number.replications <- 100000 # bigger value as impobable to match 6
...
for( replication.index in 1:number.replications ) {
    ...
    my.sample <- sample( urn.vector, 6, replace = FALSE )
   # cat(my.sample ,"\n")
       match.numbercount <- sum( my.sample >74 )
    #cat (match.numbercount,"\n")
    #use arbitary numbers 74 - 80 as numbers
    if(match.numbercount ==6) {
      result.vector[ replication.index ] =1500
    }
     if(match.numbercount ==6) {
      result.vector[ replication.index ] =150
     }
     if(match.numbercount ==4) {
      result.vector[ replication.index ] = 17
     }
    ...
    #cat ("result.vector[ replication.index ] ",result.vector[ replication.index ],"\n")
    
}

mean( result.vector)
# This doesn't really pan out as you never hit 6
