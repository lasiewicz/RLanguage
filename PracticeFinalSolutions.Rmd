...
```{r}
pgamma( 1.7, shape = 4.2, rate = 3.5 ) - pgamma(0.6, shape = 4.2, rate = 3.5)
```

```{r}
problem.4.gamma.sample <- rgamma(100000, shape = 4.2, rate = 3.5)

mean( (problem.4.gamma.sample >= 0.6) &
          (problem.4.gamma.sample <= 1.7) )
```
...
```{r}
pnorm(122.7, 108, 15) - pnorm(98.3, 108, 15)
```

```{r}
problem.4.normal.sample <- rnorm(100000, 108, 15)

mean( (problem.4.normal.sample <= 122.7) & (problem.4.normal.sample >= 98.3) )
```
...
result.vector <- rnorm( number.replications, 39, 1.8 )
...
result.vector <- rnorm( number.replications, 40, 1.8 )
...
```{r}
cat( "Expected value of S:", 100 * 2/3, "\n" )
cat( "Variance of S:", 100 * 1/18, "\n" )
```
...
for( replication.index in 1:number.replications ) {
    result.vector[ replication.index ] <- sum( rbeta(100, 2, 1) )
}
