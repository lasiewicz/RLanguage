...
curve(dexp(x,rate=1/8),lwd=2,add=TRUE)
```
...
**Answer**
```{r}
answer1<-1-exp(-((1/8)*12))
answer2<-pexp(12,rate=1/8)
cat (answer1)
cat("\n")
cat (answer2)
```
...
**Answer**
This is an exponential with rate parameter 3
...
hist(problem.2.sample,main="Problem 2 Sample Histogram",xlab="x",ylab="Density",breaks=50,prob="TRUE",col="blue")
...
```{r}
exp(-3*.5)
pexp(.5,rate=3,lower.tail = FALSE)
```
...
```{r}
mean(problem.2.sample)
var(problem.2.sample)
```
...
```{r}
problem.3.sample <- rexp(10000,rate=0.08)
hist(problem.3.sample,main="Problem 1 Sample Histogram",xlab="x",ylab="Density",breaks=50,prob="TRUE",col="red")
curve(dexp(x,rate=0.08),lwd=2,add=TRUE)
```
...
Exponential distrubition ,  rate = 1/10
upper limit 12.3
lower limit 7.5
...
mean(problem.4.sample>7.5)-mean(problem.4.sample>=12.3)
...
```{r}
problem.5.sample <- rgamma(10000,shape=4,rate=1/9)
hist(problem.5.sample,main="Problem 5 Sample Histogram",xlab="x",ylab="Density",breaks=50,prob="TRUE",col="blue")
curve(rgamma(x,rate=1/9,shape=4),lwd=2,add=TRUE)
```
...
```{r}
pgamma(62.7,rate=1/9,shape=4,lower.tail = FALSE)
```
...
```{r}
problem.6.sample <- rgamma(10000,rate=2.7,shape=6.4)
hist(problem.6.sample,main="Problem 6 Sample Histogram",xlab="x",ylab="Density",breaks=50,prob="TRUE",col="blue")
curve(rgamma(x,rate=2.7,shape=6.4),lwd=2,add=TRUE)
```
...
**Answer**
```{r}
mean(problem.6.sample)
var(problem.6.sample)
```
...
problem.7.shape<-problem.7.expectation^2/problem.7.variance
problem.7.rate<-problem.7.expectation/problem.7.variance
problem.7.shape
problem.7.rate
```
...
**Answer**
```{r}
problem.7.shape/problem.7.rate
problem.7.shape/problem.7.rate^2
```
...
```{r}
problem.7.sample <- rgamma(10000,rate=problem.7.rate,shape=problem.7.shape)
hist(problem.7.sample,main="Problem 7 Sample Histogram",xlab="x",ylab="Density",breaks=50,prob="TRUE",col="blue")
curve(rgamma(x,,rate=problem.7.rate,shape=problem.7.shape),lwd=2,add=TRUE)
```
...
```{r}
pgamma(7.1,rate=1.3,shape=5.8)-pgamma(2.3,rate=1.3,shape=5.8)
```
...
```{r}
problem.8.sample<-rgamma(10000,rate=1.3,shape=5.8)
...
```{r}
problem.8.sample <- rgamma(10000,shape=5.8,rate=1.3)
hist(problem.8.sample,main="Problem 8 Sample Histogram",xlab="x",ylab="Density",breaks=50,prob="TRUE",col="green")
curve(rgamma(x,shape=5.8,rate=1.3),lwd=2,add=TRUE)
```
