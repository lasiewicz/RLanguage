...
```{r}
1 - dhyper(0, 5, 25, 6)
cat("The probablity of at least on in the review is ", 1 - dhyper(0, 5, 25, 6))
...
```{r}
# here I iterate though values until my probablity is greater than 80 and then stop and report the value
...
# note this value matches my itterative way of solving this problem
mean( result.vector )
```
...
problem4.r<-1/43
cat("the value for r is ",problem4.r)
...
cat ("\n")
cat ("the probability that a patient will have to wait at least 1 hour" , pexp(60, problem4.r, lower.tail = FALSE))
```
...
cat( "Expected value:", patient.expected.value, "\n" )


patient.variance <- sum( (patient.cost.vector - patient.expected.value)^2 *
                           patient.prob.vector )

cat( "variance:", patient.variance, "\n" )
...
V(s)=n*var(x)
```{r}
Expected.value.sum=120*9100
Variance.sum=120*26590000
cat ("expected value of total cost" ,Expected.value.sum ,"\n")
cat ("variance value of total cost" ,Variance.sum )
...
**Answer**
This is a normal distrubtion.  It should look simular to a bell curve,  which I see in the histogram bellow.  The most common occuring costs are in the middle and as you move out to the left and right from center the outlining costs occur less and less frequently.
...
for( replication.index in 1:number.replications ) {
  result.vector[ replication.index ] <- sum( sample( c(20000,12000,8000,4000,2000), 120, replace = TRUE, prob = c(.1,.35,.25,.15,.15) ) ) 
}

mean( result.vector )
var( result.vector )
cat ("\n")
cat ("the mean is"  ,mean( result.vector ) )
cat ("\n")
cat ("the variance is"  ,var( result.vector ) )
cat ("\n")
```
```{r}

hist( result.vector,
      main = "Histogram of sum of patent costs",
      xlab = "Sum", ylab = "Density",
      prob = TRUE,
      breaks = 50,
      col = "coral1")

curve( dnorm(x, mean = patient.expected.value, sd = patient.variance), lwd = 2, add = TRUE)
...
number.replications <- 10000
result.vector.5day <- numeric( number.replications )
result.vector.4day <- numeric( number.replications )
result.vector <- numeric( number.replications )
for(replication.index in 1:number.replications) {
      # Let's model 5 days per week
  
    # First, we'll initialize the variables
  
    total.patients.5day <- 0
    total.patients.4day <- 0

    for(days.index in 1:5) {
      #sample(43:59, 1, replace = TRUE)
      total.patients.5day <- total.patients.5day + round(runif(1, min = 43, max = 59))
      # we use runif to get a random sample of patients,  since we can't have a fraction of patients we round
    }
     for(days.index in 1:4) {
      total.patients.4day <- total.patients.4day + round(runif(1, min = 54, max = 72))
       # we use runif to get a random sample of patients,  since we can't have a fraction of patients we round
     }
    if( total.patients.4day < total.patients.5day ) {
        result.vector[ replication.index ] <- TRUE
      
    }
    result.vector.5day[ replication.index ] <- total.patients.5day
    result.vector.4day[ replication.index ] <- total.patients.4day
  
}


cat( "Simulated probability  that 5 day yields more patents:", mean(result.vector), "\n" )
cat( "mean of 4 day:", mean(result.vector.4day), "\n" )
cat( "mean of 5 day:", mean(result.vector.5day), "\n" )
cat("the 5 day week yields more patients per day")
```
